...
const App = () => {
  ...
  let [informations, setInformations] = useState([]);
    ...
    setOutputText('');
      ...
      setOutputText(outputText.substring(0, outputText.lastIndexOf(space)));
		}
		else {
			alert('empty!');
		}
      ...
      <div className="xl:max-w-[80rem] mx-auto flex flex-col xl:gap-3 xl:py-3 h-screen">
        ...
        <main className="flex flex-1 overflow-hidden xl:gap-3">
          <section className="flex flex-col w-1/4 xl:gap-3">
            ...
            <h1 className="tracking-wider text-white bg-green-500 text-center leading-[3rem] rounded">
              Accounts Added
            </h1>
              ...
              {informations.length > 0 ? informations.map((information, i) => (
                <div className="text-white" key={i}>
                  <div className="flex overflow-hidden border-b border-white rounded-tl">
                    ...
                    <input placeholder="Give the account a name (e.g., Sina)" className="sticky top-0 z-10 w-full font-bold bg-lime-500 xl:leading-10 xl:px-3 focus:outline-none placeholder:text-gray-100" onChange={(event)=> {
                          const newInformations = [...informations];
                  ...
                  <ul className="text-xs">
                    {information.accounts.map((account, j) => (
                      <li className="flex border-b border-white xl:leading-8" key={j}>
                        ...
                        <button className="size-8 flex justify-center items-center bg-lime-500 hover:bg-lime-400 transition-colors duration-[250ms]" onClick={ () => {
                          ...
                          <svg className="w-auto h-1/2 fill-white" xmlns="http://www.w3.org/2000/svg" fill="currentColor" color="" viewBox="5 3 14 18"><path d="M0 0h24v24H0z" fill="none"></path><path d="M0 0h24v24H0V0z" fill="none"></path><path d="M6 19c0 1.1.9 2 2 2h8c1.1 0 2-.9 2-2V7H6v12zm2.46-7.12l1.41-1.41L12 12.59l2.12-2.12 1.41 1.41L13.41 14l2.12 2.12-1.41 1.41L12 15.41l-2.12 2.12-1.41-1.41L10.59 14l-2.13-2.12zM15.5 4l-1-1h-5l-1 1H5v2h14V4z"></path></svg>
                        </button>
                      </li>
                    ))}
                  </ul>
                  <button className="flex items-center justify-center w-full h-10 transition-colors duration-[250ms] bg-lime-500 hover:bg-lime-400" onClick={ () => {
            ...
            {informations.length > 0 && (
              <button className="h-12 mt-auto w-full flex xl:gap-2 justify-center items-center bg-red-600 hover:bg-red-700 transition-colors duration-[250ms] rounded text-white tracking-widest" onClick={ deleteEntries }>
                <svg className="w-auto h-1/2 fill-white" xmlns="http://www.w3.org/2000/svg" fill="currentColor" color="" viewBox="5 3 14 18"><path d="M0 0h24v24H0z" fill="none"></path><path d="M6 19c0 1.1.9 2 2 2h8c1.1 0 2-.9 2-2V7H6v12zM19 4h-3.5l-1-1h-5l-1 1H5v2h14V4z"></path></svg>
                <span>
                  Delete All Accounts
                </span>
          ...
          <section className="flex flex-col flex-1 xl:gap-3">
            <div className="flex flex-col flex-1 overflow-hidden border border-gray-400 rounded focus-within:border-black">
              <select className="w-full h-12 tracking-wider text-black bg-slate-300 focus:outline-none xl:px-3" onChange={(event)=> {
              ...
              <textarea className="tracking-widest resize-none bg-slate-100 size-full focus:outline-none xl:p-3 placeholder:text-slate-500 focus:bg-white transition-colors duration-[250ms]" onChange={(event)=> {
                  setInputText(event.target.value)
                }
              } value={inputText} placeholder="Your message!"></textarea>
            </div>
            <button className="h-12 w-full flex justify-center items-center bg-slate-500 hover:bg-slate-600 transition-colors duration-[250ms] rounded" onClick={ convert }>
            ...
            <div className="flex flex-col flex-1 overflow-hidden border border-gray-400 rounded focus-within:border-black">
              <p className="tracking-widest bg-slate-100 size-full xl:p-3">
                {outputText}
              </p>
              <select className="w-full h-12 tracking-wider text-black bg-slate-300 focus:outline-none xl:px-3" onChange={(event) => {
          ...
          <section className="flex flex-col w-1/4 xl:gap-3">
            <h1 className="tracking-wider text-white bg-orange-600 text-center leading-[3rem] rounded">
              Accounts Mentioned
            </h1>
