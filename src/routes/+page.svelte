	...
	<main class="flex flex-1 overflow-hidden xl:gap-3">
		<section class="flex flex-col w-1/4 xl:gap-3">
			<div class="flex xl:gap-3">
				<button class="w-1/2 tracking-wider text-white bg-green-600 transition-colors duration-[250ms] hover:bg-green-500 text-center leading-[3rem] rounded" onclick={ localStorageHandler }>
					Export
				</button>
				<button class="w-1/2 tracking-wider text-white bg-green-600 transition-colors duration-[250ms] hover:bg-green-500 text-center leading-[3rem] rounded">
					Import
				</button>
			</div>
			<h1 class="tracking-wider text-white bg-lime-500 text-center leading-[3rem] border-b border-white rounded">
				Accounts Added
			</h1>
				...
				{#each informations as information, i}
					<div class="tracking-widest text-white">
						{#if information.accounts.length}
							<input placeholder="Give the account a name (e.g., Sina)" class="sticky top-0 z-10 w-full border-b border-white bg-lime-400 xl:leading-10 xl:px-3 focus:outline-none placeholder:text-gray-100" bind:value={information.name}>
						{/if}
						<ul class="text-xs">
		...
		<section class="w-1/4">
			<h1 class="tracking-wider text-white bg-orange-600 text-center leading-[3rem] border-b border-white rounded-t">
				Accounts Mentioned
			</h1>
			<ul>
				{#each nameEntries as nameEntry}
					<li class="tracking-widest text-white bg-orange-400 border-b border-white xl:leading-8 xl:px-3">
						{nameEntry}
					</li>
				{/each}
			</ul>
